...
public class MainController implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        // Add a listener to ensure Leaflet is fully loaded
        webEngine.getLoadWorker().stateProperty().addListener((obs, oldState, newState) -> {
            if (newState == Worker.State.SUCCEEDED) {
                System.out.println("Leaflet loaded successfully.");
            }
        });
    }
    ...
    private void handleAddProjet() {
        ...
        double latitude = 36.8065; // Exemple de latitude
        double longitude = 10.1815; // Exemple de longitude
        double[] latitudesZoneImpact = {36.8065, 36.8066}; // Exemple de latitudes de la zone d'impact
        double[] longitudesZoneImpact = {10.1815, 10.1816}; // Exemple de longitudes de la zone d'impact
        ...
        for (Node node : projetCardContainer.getChildren()) {
            if (node instanceof AnchorPane) {
                ...
                if (projetWrapper != null) {
                    ...
                    if (projetWrapper.getLatitude() == 0.0 || projetWrapper.getLongitude() == 0.0) {
                        System.out.println("Skipping project due to invalid coordinates: " + projetWrapper.getProjet().getNom());
                        continue;
                    }

                    // Execute JavaScript to add a marker
